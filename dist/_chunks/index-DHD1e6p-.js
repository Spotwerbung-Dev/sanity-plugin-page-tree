import { orderBy, sortBy, omit, groupBy } from 'lodash';
...
const flatMapPageTree = pages => pages.flatMap(page => page.children ? [omit(page, "children"), ...flatMapPageTree(page.children)] : page);
...
const getPublishedAndDraftRawPageMetadata = (config, pages) => {
  const publishedPages = groupBy(pages.filter(p => !p._id.startsWith(DRAFTS_PREFIX)), p => p._id);
  const draftPages = groupBy(pages.filter(p => p._id.startsWith(DRAFTS_PREFIX)), p => getSanityDocumentId(p._id));
...
export { DRAFTS_PREFIX, findPageTreeItemById, flatMapPageTree, getAllPageMetadata, getAllRawPageMetadataQuery, getLanguageFieldName, getRawPageMetadataQuery, getRootPageSlug, getSanityDocumentId, mapRawPageMetadatasToPageTree };
//# sourceMappingURL=index-DHD1e6p-.js.map
